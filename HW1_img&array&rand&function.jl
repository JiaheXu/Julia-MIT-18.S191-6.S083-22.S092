...
function create_bar()
	hundred_zeros = zeros(100)
	n = length(hundred_zeros) ÷ 2
	hundred_zeros[n-20÷2:n+20÷2] .= 1
	return hundred_zeros
end
...
# ╔═╡ 2a5e028c-2c8e-42d9-999c-aa3a5d90431e
head_red = get_reds( philip_head)

# ╔═╡ c432f205-d590-4c19-9a0c-18348f64273d
pic = [value_as_color(value) for value in head_red]
...
# ╔═╡ 01cbdb33-3a22-40a0-8b5c-afe81c542ad2
function get_green(pixel::AbstractRGB)
	return pixel.g
end

# ╔═╡ 6b1f48a9-c797-4ba7-b131-0aba9f5c1579
function get_blue(pixel::AbstractRGB)
	return pixel.b
end

# ╔═╡ e82cea9a-4afb-4574-aadc-905995c1f160
function get_greens(image::AbstractMatrix)
	
	return [ get_green(pixel) for pixel in image ]
end

# ╔═╡ d994e178-78fd-46ab-a1bc-a31485423cad
function get_blues(image::AbstractMatrix)
	
	return [ get_blue(pixel) for pixel in image ]
end
...
philip_inverted = [ invert(pixel) for pixel in philip_head] # replace `missing` with your code!
...
function quantize(x::Number)
	return floor(x, digits=1) # digits = 1 -> 0.1
end
...
function quantize(image::AbstractMatrix)
	#pic = [quantize(pixel) for pixel in image]
	#return pic
...
The image is unrecognisable with intensity 5
As the magnitude of the noise increases beyond the range of the RGB values, each pixel might move very far away from its original value. Hence, the image becomes unrecognisable.
"""
