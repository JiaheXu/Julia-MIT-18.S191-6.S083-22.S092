...
md"_Homework 10 Spring 2021, version 1_"
...
A more negative B makes it less sensitive to CO2 change
"""
...
ΔT keep increasing over time to infinity.
"""

# ╔═╡ 56b68356-2601-11eb-39a9-5f4b8e580b87
@bind reveal_nonnegative_B_answer CheckBox()
...
histogram(ECS_samples, label=nothing, xlabel="B [W/m²/K]", ylabel="samples")
...
mean(ECS_samples), ECS(B=mean(B_samples)), count(ECS_samples .> 2.64953)/length(ECS_samples)*100
# higher expected warming by 0.71 degrees
# 49.58% chance that ECS(B) lies above ECS(mean(B))
...
Worse, higher than expected temperature
"""
...
# ╔═╡ c0c131e3-20d6-401f-aa36-6dd5e1426539
t

# ╔═╡ 545ca401-0ff1-482f-89fe-795e9a0ce7d4
begin
	val = Model.CO2_RCP85(first(t)) * 2
	findfirst(x -> Model.CO2_RCP85(x) , t )
end

# ╔═╡ 50ea30ba-25a1-11eb-05d8-b3d579f85652
expected_double_CO2_year = findfirst(x -> Model.CO2_RCP85(x) >=2*Model.CO2_RCP85(first(t)) , t ) + first(t)
...
#RCP2.6, 46.13% be more than 2 degrees warming
let
	response = temperature_response.(Model.CO2_RCP26, B_samples)
	count(response .> 16) / length(response) * 100
end

# ╔═╡ aba079b3-c0e1-4321-8fad-15133a6e9a36
#RCP8.5, 62.75% be more than 2 degrees warming
let
	response = temperature_response.(Model.CO2_RCP85, B_samples)
	count(response .> 16) / length(response) * 100
end
...
@bind log_CO2 Slider(log(10, CO2min):0.1:log(10, CO2max), show_value=true)
...
# ╔═╡ 42241741-8ea0-4eff-b0d5-2f88241092d2
function equilibrium_temperature(CO2)
	ebm = Model.EBM(Tneo, 0., 5., t -> CO2)
	Model.run!(ebm)
	return last(ebm.T)
end
...
co2_to_melt_snowball = let
	# res = equilibrium_temperature.(CO2min:10:CO2max)
	co2_melt = missing
	for i ∈ CO2min:10:CO2max
		if equilibrium_temperature(i) ≥ 10
			co2_melt = i
			break
		end
	end
