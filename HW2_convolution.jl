...
# ╔═╡ d496e930-0cca-48cf-a5ee-dd9513e3c4a8
using LinearAlgebra
...
# ╔═╡ 78da853a-f201-4168-9f80-d0bd1d256a91
∈

# ╔═╡ 8d583ce5-f44b-40a7-8a09-3d6308b78cca
md"""Start typing, ie: \alpha , select and press  TAB ."""
...
# ╔═╡ 763f0e35-c4a3-4b1e-aa56-8b926d7e3ad2
...
function gaussian_kernel_1D(n; σ = 1)
	l = (n - 1) ÷ 2
	_kernel =  [ 1 / √(2 * π * σ^2) * exp(-x^2 / (2 * σ^2)) for x ∈ -l:+l ]
...
function box_blur(v::AbstractArray, l)
	side = 2 * l + 1
	extended = [extend(v, j) for i ∈ 1:length(v) for j ∈ i-l:i+l]
	return [ mean(extended[i:i+side-1]) for i ∈ 1:side:length(extended) ]
end
...
blured_v = box_blur(v, l_box)

# ╔═╡ 7d80a1ea-a0a9-41b2-9cfe-a334717ab2f4
colored_line(blured_v)
...
function convolve(v::AbstractVector, k)
	...
	l_k= length(k)
	l = (l_k - 1) ÷ 2
	extended_v = [extend(v, j) for i ∈ 1:length(v) for j ∈ i-l:i+l]
	return [dot(extended_v[i:i+l_k-1],  k) for i ∈ 1 : l_k : length(extended_v) ]
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	rows, cols = size(M)
	row_offset, col_offset = (size(K) .- 1) .÷ 2
	extended_M = [ extend(M, i, j) for (i,j) ∈ Iterators.product(1-row_offset:rows+row_offset, 1-col_offset:cols+col_offset) ]
	# broadcast * between submatrices and K and then sum for each entry of M
	hₖ, lₖ = size(K)
	return [ sum(extended_M[i:i+hₖ-1, j:j+lₖ-1] .* K) for i ∈ 1:size(extended_M, 1)-(hₖ-1), j ∈ 1:size(extended_M, 2)-(lₖ-1) ]
end
...
function with_gaussian_blur(image; σ=3, l=5)
	G(x, y, σ=1) = 1 / (2π * σ) * exp(-(x^2 + y^2) / (2σ^2))
	n = 2 * l + 1
	kernel = [ G(i, j) for i ∈ 1:n, j ∈ 1:n ]
	kernel ./= sum(kernel)
