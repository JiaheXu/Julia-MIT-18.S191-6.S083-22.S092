...
student = (name = "Jiahe Xu", kerberos_id = "jxu")
...
Submission by: **_$(student.name)_** ($(student.kerberos_id)@jhu.edu)
"""
...
function newton_sqrt(x, error_margin=0.01, a=x / 2) # a=x/2 is the default value of `a`
	if(abs( a*a - x) < error_margin)
		return a
	end
	a=( x/a+a)/2
	return  newton_sqrt(x, error_margin,  a)
end
