...
# ╔═╡ 00c0fbd4-9086-49af-a935-51f7d4d53919
...
double_letters = alphabet[findall(!iszero, diag(sample_freq_matrix))] # replace with your answer
...
row_col_answer = md"""
The sum of each row is the marginal probability that the letter corresponding to the row appears as the first letter of a letter pair.
	
The sum of each row is the marginal probability that the letter corresponding to the column appears as the second letter of a letter pair.
"""
...
function completion_cache(grams)
	...
	for substrings in grams
		key = substrings[1:end-1]
		value = substrings[end]
		if haskey(cache, substrings[1:end-1])
			cache[key] = vcat(cache[key], value)
		else
			cache[key] = [value]
		end
	end
