...
# ╔═╡ ce2274e1-a3a0-46bf-89f7-0bf84cb0e34c
(49*(1/49))
...
function Base.getindex(M::FirstRankOneMatrix, i, j)
	return M.v[i]*M.w[j]
	#return getindex(M.v, i)*getindex(M.w, j)
end
...
# ╔═╡ ba33b153-c3fe-4682-9365-484befd7ae71
begin
...
# ╔═╡ 429b3d1b-c248-4e9c-a84f-8fedba8b2ab4
RankOneMatrix(1.0:10.0)
...
# ╔═╡ 124dcb1e-af11-44df-b52f-61e1cb0bd792
N = RankOneMatrix(1.0:10.0)
...
function numerical_rank(A::AbstractMatrix; tol=1e-5)
	...
	return count(i->(i>tol),LinearAlgebra.svd(A).S)
end
...
function k_rank_ones(k, m, n)
	...
	A = sum(RankOneMatrix(rand(m),rand(n)) for i in 1:k) 
