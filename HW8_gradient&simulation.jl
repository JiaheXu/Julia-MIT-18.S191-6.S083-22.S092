...
# ╔═╡ fdff90f4-0d32-4677-b8c2-24ba486bf3af
...
function tangent_line(f, a, h)
	newf = x -> finite_difference_slope(f,a,h)*(x-a) + f(a)
	return newf
end
...
function euler_integrate(fprime::Function, fa::Number, 
	...
	for i in 1:length(T) - 1
        push!(ans, euler_integrate_step(fprime,ans[i],T[i+1],h))
    end 
...
md"""
β = $(@bind β Slider(0:.001:1.0, show_value = true, default=0.9))
"""

# ╔═╡ 9e97ded1-0712-498f-b2e5-426e59d102c3
md"""
γ = $(@bind γ Slider(0:.01:1, show_value = true, default=0.1))
"""

# ╔═╡ 0d0f73a9-da20-4aff-83c6-9931196963fc
r = 0:0.01:500

# ╔═╡ e5c09bf7-bc06-43f0-bac0-58487d26eac0
interactive_SIR = euler_SIR(β,γ,[0.99,0.01,0.00],r)

# ╔═╡ 5e16462f-8512-4298-a96f-521f43a74b44
plot_sir!(plot(), r, interactive_SIR)
...
function ∂x(f::Function, a, b)
	...
	return ForwardDiff.derivative( tmp -> f(tmp,b) , a )
end
...
function gradient_descent_1d(f, x0; η=0.01, N_steps=1000)
	...
	for i in 1:N_steps
		...
		if(round(next_step, digits=4) == round(ans[i], digits=4))
			return next_step # finish early if found answer to enough sig figs 
		else 
        	push!(ans, next_step)
		end
	end
...
when the result doesn't change much in around 50 steps
"""
...
function gradient_descent_2d(f, x0, y0; η=0.01)
	...
	N_steps = 10000
	digits_to_Round = 4
    ans = [gradient_descent_2d_step(f, x0, y0, η = η)]
    for i in 1:N_steps
...
function loss_sir(β, γ)
	...
	ans = zeros(3,1)
	theo = euler_SIR(β, γ,[0.99, 0.01, 0.00], spatial_T)
	for i in spatial_T
...
# ╔═╡ eb215c0e-55c3-479e-9f28-59478cd18ef4
gradient_descent_2d_step(loss_sir, 0.01, 0.01, η = (0.01/1000000))

# ╔═╡ 6bcab1ce-7c0b-4ddf-87e2-2be889156cda
begin
	f= loss_sir
	x0 = guess_β 	# β = 0.0188 ; Actual β = 0.01865
	y0 = guess_γ 	# γ = 0.0022 ; Actual γ = 0.00205
	η = x0/1000000
	N_steps = 1000
	digits_to_Round = 6

    ans = [gradient_descent_2d_step(f, x0, y0, η = η)] # initialize
	for i in 1:N_steps
		new_step = gradient_descent_2d_step(f,ans[i][1],ans[i][2], η = η)
		if round.(new_step,digits=digits_to_Round) == round.(ans[i],digits=digits_to_Round)
			break # new_step
...
# ╔═╡ 7f27cde1-7074-4d0c-b9c8-87e1dddf1c95
let
	p = plot()
	plot_sir!(p, spatial_T, spatial_results, label="spatial", opacity=.7)
...
# ╔═╡ c650d900-4cfa-4e7c-84bd-3c843a7c570d
Tup = (db = abs(found_β - 0.019), dc = abs(found_γ - 0.0026))
