...
function random_seam(energies, starting_pixel::Int)
	m,n = size(energies)
	# you can delete the body of this function - it's just a placeholder.
	random_seam(size(energies)..., starting_pixel)
end

# ╔═╡ 6c49d47c-9cd6-4ceb-97b9-b773ea87a800
function greedy_seam(energies, starting_pixel::Int)
	rows, columns = size(energies)
	seams = zeros(Int, rows)
	
	seams[1] = starting_pixel
	
	for i ∈ 2:rows
		_, seam = findmin(energies[i, max(seams[i-1]-1, 1):min(seams[i-1]+1, columns)])
		seam -= 2
		if seams[i-1] == 1
			seam += 1
		end
		seams[i] = seam + seams[i-1]
	end
	return seams
end
...
function least_energy(energies, i, j)
	...
	for k in 1:3
		new_j = j + change[k]
		if( 1<=new_j && new_j<=n )
			(new_cost,new_ind) = least_energy( energies,i+1,new_j )
			new_cost += energies[i,j]
...
function recursive_seam(energies, starting_pixel)
	...
	for row ∈ 1:m-1
		pixel = seam[row]
		_, next_pixel = least_energy(energies, row, pixel)
		seam[row+1] = next_pixel
	end
	return seam
end
...
function memoized_least_energy(energies, i, j, memory::Dict)
	...
	if haskey(memory, (i, j))
		...
		for k in 1:3
			new_j = j + change[k]
			if( 1<=new_j && new_j<=n )
				(new_cost,new_ind) = memoized_least_energy( energies,i+1,new_j ,memory)
				new_cost += energies[i,j]
...
function memoized_recursive_seam(energies, starting_pixel)
	...
	seam = zeros(Int, m)
	seam[1] = starting_pixel
	_ = memoized_least_energy(energies, 1, starting_pixel, memory)
	for row ∈ 1:m-1
		
		column = seam[row]
		_,next_column = memory[row,column]
...
function matrix_memoized_least_energy(energies, i, j, memory)
	...
	if memory[i,j]!= nothing
		...
		for k in 1:3
			new_j = j + change[k]
			if( 1<=new_j && new_j<=n )
				(new_cost,new_ind) = matrix_memoized_least_energy( energies,i+1,new_j ,memory)
				new_cost += energies[i,j]
...
function matrix_memoized_seam(energies, starting_pixel)	
	m, n = size(energies)
	memory =  Array{Union{Nothing,Tuple{Float64,Int} }}(nothing, m, n)

	
	seam = zeros(Int, m)
	seam[1] = starting_pixel
	println("$starting_pixel")
	_ = matrix_memoized_least_energy(energies, 1, starting_pixel, memory)
	println("orz")
	for row ∈ 1:m-1
...
# ╔═╡ e9c58947-865c-42ec-bf93-34865389bd6b
matrix_memoized_seam(grant_example, 4)
...
function least_energy_matrix(energies)
	m, n = size(energies)
	minimal_energy_to_bottom = zeros(m, n)
	minimal_energy_to_bottom[end, :] = energies[end, :]
	
	for i ∈ m-1:-1:1
		@views for j ∈ 1:n
			neighbours = j .+ [-1, 0, 1]
			clamp!(neighbours, 1, n)
			min_neighbour = minimum([energies[i+1, k] for k in neighbours])
			minimal_energy_to_bottom[i, j] = energies[i, j] + min_neighbour
		end
	end
	return minimal_energy_to_bottom
end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	seam[1] = starting_pixel
	for row ∈ 1:m-1
		next_columns = seam[row] .+ [-1, 0, 1]
		clamp!(next_columns, 1, n)
		next_column_index = @views argmin([least_energies[row+1, next_columns[1]],
							least_energies[row+1, next_columns[2]],
							least_energies[row+1, next_columns[3]]])
		seam[row+1] = next_columns[next_column_index]
	end
	return seam
