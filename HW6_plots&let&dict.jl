...
function counts2(data::Vector)
	...
	cnts = counts(data)
    ks = collect(keys(cnts))
    vs = collect(values(cnts))
    perm_ks = sortperm(ks)
    perm_vs = sortperm(vs)
	
	return ks[perm_ks],vs[perm_ks]
end
...
function probability_distribution(data::Vector)
    ...
    total = sum(vec[2]) # sum of all samples

    for i in 1:size(vec[1],1) # for each keys, replace freq with prob
...
m = sum(large_experiment) / length(large_experiment)
...
function PlotExp(p::Real, N::Integer)
	...
	plot(xs, ps, alpha=0.5, leg=true)	
	vline!([meanExp(p)],ls=:dash)
end 

# ╔═╡ 501bcc30-f96f-42e4-a5aa-09a4138b5b72
PlotExp(prob,Sample)

# ╔═╡ 60c23a9e-81ba-4967-abc7-fb6ef65bbfc6
@bind prob_P_vs_tP Slider( 0.01 : 0.01 : 0.3 )
...
```math
\langle \tau(p) \rangle = e^p
```\langle \tau(p) \rangle = my \cdot answer \cdot here e^p
"""
...
# ╔═╡ 9d71ab7b-d303-4072-bf3d-08f304a8b1f7
Ps

# ╔═╡ 063e07f6-9c83-49a1-b8e0-465c3c9138b0
cumSum1 = cumulative_sum(Ps)

# ╔═╡ 8dbe3c14-76a5-4176-b276-7fb5f37ad19d
function cum_sumAnal(p::Real,n::Int)
	a₁ = p 	
	r = 1 - p	
	return (a₁*(1-r^n))/p 
end 

# ╔═╡ c83f9341-f2b0-4c73-934f-e55279a51e55
function cum_sumAnal2(p::Real,n::Int)
	return 1-(1-p)^n
end 

# ╔═╡ 003f3678-56ec-45f5-9d43-7825ed52ad2d
cumSum2 = [cum_sumAnal(0.25,i) for i in 1:50]

# ╔═╡ 5cabcfb8-6441-465a-a8aa-488534df27ee
cumSum3 = [cum_sumAnal2(0.25,i) for i in 1:50]

# ╔═╡ a6354695-9bbf-4b5b-bf86-72aca7ea1e56
cumSum1 - cumSum2

# ╔═╡ 0bbd067f-1546-45df-afb7-a76f8683b47b


# ╔═╡ 326df1d3-f78b-4781-9442-e18f0f110e8d
...
```math
S_n = \frac{a_1(1-r^n)}{1-r}, a_1 = p, a_2 = p(1-p)
```

```math
r = \frac{a_2}{a_1} = \frac{p (1 - p)}{p} = 1-p
```

```math
C_n := \sum_{k=1}^n P_k = \frac{p(1-(1-p)^n)}{1-(1-p)} = 1-(1-p)^n
```
```
"""
...
function geometric_bin(u::Real, p::Real)
	...
	return 1+floor(log(1-u) / log(1-p))
end
...
samples = [ geometric_fast(probs) for i in 1:10000]

# ╔═╡ d2ad404d-bdef-46a0-ac1d-b575a1023593
histogram(samples)

# ╔═╡ 92d72ac6-3474-4f25-b86f-df7c441ff6ab


# ╔═╡ 25f392ef-6400-4bd1-9198-ecba60ab5cc9
